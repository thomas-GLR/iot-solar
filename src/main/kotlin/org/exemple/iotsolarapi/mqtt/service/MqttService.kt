...
class MqttService(
    ...
    fun subscribeToTopics() {
        ...
        if (mqttClient.isConnected) {
            ...
            try {
                logger.info("Souscription au topic: {}", TOPIC_RESISTANCE_ACK)

                mqttClient.subscribe(TOPIC_RESISTANCE_ACK, 1) { topic, message ->
                    logger.info("Message reçu sur {}", topic)
                    resistanceStateService.updateResistanceStateFromEsp32Ack()
                }
            } catch (e: Exception) {
                logger.error("Erreur lors de la souscription au topic {}: {}", TOPIC_RESISTANCE_ACK, e.message)
            }
        }
    }
